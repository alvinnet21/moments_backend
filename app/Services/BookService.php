...
class BookService
{
    ...
    public function createAdminOff(array $data): Book
    {
        // Admin-requested off/blocked time: create FINISHED booking with dummy customer info
        ...
        $book->time_slot = $data['time_slot'];
        $book->status = $status;
        $book->customer_name = $data['customer_name'] ?? 'Test Customer';
        $book->customer_email = $data['customer_email'] ?? 'test@example.com';
        $book->customer_phone = $data['customer_phone'] ?? '0000000000';
        $book->save();
        return $book->fresh(['employee']);
    }
